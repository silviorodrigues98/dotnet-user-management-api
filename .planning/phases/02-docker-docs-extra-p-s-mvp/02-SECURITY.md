...
- **E2E do compose (validado em 2026-08-20):** Docker Engine 29.7.2 + Compose v5.5.0 instalados no WSL2. `docker compose up --build` sobe API + PostgreSQL 16 com healthcheck; fluxo E2E validado: register 201 (duplicado também 201 — uniforme, anti-enumeração T-01-10), login 200 com JWT, GET /api/users 401 sem token e 200 com token. Persistência confirmada no volume `postgres_data` (usuário sobreviveu a rebuild do container da API).
